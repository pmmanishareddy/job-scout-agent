#!/usr/bin/env python3
"""
Job Scout v4 — Searches job boards for PM/APM roles, scores against profile, outputs daily digest.
"""
import json, re, urllib.request, urllib.parse, time, html, os, logging
...
# ---------------------------------------------------------------------------
# Load profile from YAML (fallback to inline defaults if pyyaml not installed)
# ---------------------------------------------------------------------------
def load_profile():
    yaml_path = os.path.join(PROJECT_DIR, "profile.yaml")
    try:
        import yaml
        ...
        return {
            ...
            "high_weight_keywords": p.get("scoring", {}).get("high_weight_keywords", []),
            "ai_bonus_keywords": p.get("scoring", {}).get("ai_bonus_keywords", []),
            "technical_bonus_keywords": p.get("scoring", {}).get("technical_bonus_keywords", []),
            "too_senior_titles": [t.lower() for t in p.get("seniority", {}).get("too_senior", [])],
            "negative_keywords": p.get("scoring", {}).get("negative_keywords", []),
            "email_to": p.get("notification", {}).get("email_to", ""),
        }
    except ImportError:
        log.warning("pyyaml not installed — using inline profile")
    except FileNotFoundError:
        log.warning(f"{yaml_path} not found — using inline profile")
    return {
        "target_titles": [
        ...
        "high_weight_keywords": [
            "product manager", "product owner", "product management", "associate product manager",
            "APM", "junior product manager", "PRD", "roadmap", "cross-functional", "stakeholder",
            "agile", "scrum", "backlog", "sprint", "user research", "discovery",
            "go-to-market", "GTM", "prioritization"
        ],
        "ai_bonus_keywords": [
            "LLM", "Claude", "GPT", "OpenAI", "AI", "ML", "machine learning",
            "prompt", "RAG", "agents", "agentic", "responsible AI", "generative",
            "artificial intelligence", "NLP", "deep learning", "chatbot"
        ],
        "technical_bonus_keywords": ["API", "system design", "SQL", "Python", "SaaS", "platform"],
        "too_senior_titles": [
            "senior product manager", "lead product manager", "principal product manager",
            "staff product manager", "group product manager", "director of product",
        ...
        "negative_keywords": [
            ...
            "security clearance required", "10+ years PM experience required",
            "8+ years product management", "7+ years product management",
            "5+ years product management required"
        ],
...
def fetch_html(url, timeout=15):
    ...
    try:
        ...
        log.warning(f"Fetch error ({type(e).__name__}) for {url}")
    return ""
...
def fetch_json(url, timeout=15):
    ...
    try:
        ...
        log.warning(f"JSON fetch error ({type(e).__name__}) for {url}")
        return None
...
def extract_jobs(keyword, location, market):
    ...
    time_filter = "r604800" if market == "dubai" else "r86400"
    url = f"https://www.linkedin.com/jobs/search?keywords={kw_enc}&location={loc_enc}&f_TPR={time_filter}"

    page_html = fetch_html(url, timeout=20)
    if not page_html:
        return []

    jobs = []
    cards = re.split(r'<div[^>]*class="[^"]*base-search-card[^"]*job-search-card[^"]*"', page_html)[1:]
    ...
    for card_html in cards[:30]:
        ...
        subtitle = re.search(r'base-search-card__subtitle[^>]*>(.*?)</h4>', card_html, re.DOTALL)
        if subtitle:
            links = re.findall(r'<a[^>]*>(.*?)</a>', subtitle.group(1), re.DOTALL)
            if links:
                job["company_name"] = strip_html(links[0])
            else:
                job["company_name"] = strip_html(subtitle.group(1))
        else:
...
def fetch_linkedin_jd(job):
    """Fetch full JD via LinkedIn guest API."""
    job_id = job.get("job_id", "")
    if not job_id:
        return
    url = f"https://www.linkedin.com/jobs-guest/jobs/api/jobPosting/{job_id}"
    page_html = fetch_html(url, timeout=12)
    ...
    if len(text) > 100:
        job["job_description"] = text[:4000]

# ---------------------------------------------------------------------------
# RemoteOK — JSON API
# ---------------------------------------------------------------------------
def extract_jobs_remoteok():
    """Fetch RemoteOK jobs via their public JSON API."""
    data = fetch_json("https://remoteok.com/api", timeout=20)
    ...
    for item in data[1:]:  # first element is metadata
        ...
        jobs.append({
            ...
            "market": "us",
            "source": "RemoteOK",
            "application_url": item.get("url", f"https://remoteok.com/l/{item.get('id', '')}"),
...
def extract_jobs_remotive():
    """Fetch Remotive jobs via their public API."""
    data = fetch_json("https://remotive.com/api/remote-jobs?category=product&limit=30")
    if not data:
        return []
    jobs = []
...
def extract_jobs_himalayas():
    """Fetch Himalayas jobs via their public API."""
    data = fetch_json("https://himalayas.app/jobs/api?categories=product-management&limit=30")
    if not data:
        return []
    ...
    if not isinstance(jobs_list, list):
        return []
    jobs = []
    for item in jobs_list[:30]:
        title = item.get("title", item.get("name", ""))
        company = item.get("companyName", item.get("company_name", item.get("company", {}).get("name", "Unknown")))
        slug = item.get("slug", item.get("id", ""))
        jobs.append({
            "job_id": f"hml_{item.get('id', slug)}",
            "job_title": title,
            "company_name": company if isinstance(company, str) else "Unknown",
            "location": item.get("location", "Remote"),
            "work_model": "remote",
            "market": "us",
            "source": "Himalayas",
            "application_url": item.get("applicationUrl", item.get("url", f"https://himalayas.app/jobs/{slug}")),
            "date_posted": str(item.get("pubDate", item.get("published_at", "")))[:10],
            "salary": None,
            "job_description": strip_html(item.get("description", item.get("excerpt", "")))[:3000],
        })
    return jobs

# ---------------------------------------------------------------------------
# Bayt — Cloudflare-blocked, kept as stub with warning
# ---------------------------------------------------------------------------
def extract_jobs_bayt(keyword, market="dubai"):
    """Bayt.com — blocked by Cloudflare. Returns empty; needs Playwright."""
    kw_slug = keyword.lower().replace(" ", "-")
    url = f"https://www.bayt.com/en/uae/jobs/{kw_slug}-jobs/"
    page = fetch_html(url, timeout=20)
    if not page or "challenge" in page[:2000].lower():
        log.info(f"Bayt blocked (Cloudflare) for '{keyword}'")
        return []
    jobs = []
    ...
    for card in cards[:20]:
        ...
        jobs.append({
            "job_id": f"bayt_{jid}", "job_title": title, "company_name": company,
            "location": "Dubai, UAE", "work_model": "unknown", "market": market,
            "source": "Bayt", "application_url": href, "date_posted": "",
            "salary": None, "job_description": "",
        })
    return jobs
...
def extract_jobs_gulftalent(keyword, market="dubai"):
    ...
    for card in cards[:20]:
        ...
        jid = re.sub(r'[^a-zA-Z0-9]', '_', href[-40:]) if href else title[:20]
        jobs.append({
            "job_id": f"gt_{jid}", "job_title": title, "company_name": company,
            "location": "UAE", "work_model": "unknown", "market": market,
            "source": "GulfTalent", "application_url": href, "date_posted": "",
            "salary": None, "job_description": "",
        })
    return jobs

# ---------------------------------------------------------------------------
# Generic HTML scraper (The Product Folks, Mind the Product)
# ---------------------------------------------------------------------------
def extract_jobs_generic_html(board_name, url, market, id_prefix):
    ...
    for href, link_text in links:
        ...
        jobs.append({
            "job_id": f"{id_prefix}_{jid}", "job_title": text, "company_name": "Unknown",
            "location": "Remote" if market != "dubai" else "Dubai",
            "work_model": "unknown", "market": market, "source": board_name,
            "application_url": href, "date_posted": "", "salary": None, "job_description": "",
        })
    return jobs[:20]
...
def score_job(title, company, description, location, market):
    ...
    # --- Title Match (30 points) --- substring containment, not exact equality
    target_lower = [t.lower() for t in profile["target_titles"]]
    if any(t in title_lower for t in target_lower):
        total += 30; breakdown.append("Title match: +30")
    ...
    # --- Skills Match (40 points) --- +3 per keyword hit
    if description:
        hw = min(sum(3 for kw in profile["high_weight_keywords"] if kw.lower() in desc_lower), 18)
        if hw: total += hw; breakdown.append(f"High-weight keywords: +{hw}")

        ai = min(sum(3 for kw in profile["ai_bonus_keywords"] if kw.lower() in desc_lower), 15)
        if ai: total += ai; breakdown.append(f"AI/ML keywords: +{ai}")

        tech = min(sum(3 for kw in profile["technical_bonus_keywords"] if kw.lower() in desc_lower), 12)
    ...
    if market == "dubai":
        ...
        if remote_worldwide:
            ...
            total -= 15; breakdown.append(f"{market.upper()} remote unclear: -15")
    ...
    elif any(t in title_lower for t in ["associate product manager", "apm", "junior product manager", "associate pm", "assistant product manager"]):
        total += 15; breakdown.append("APM/entry-level match: +15")
    else:
        total += 10; breakdown.append("Seniority OK: +10")

    # --- Negative signals (capped at -25 total) ---
    if description:
        neg_total = 0
        for neg in profile["negative_keywords"]:
            if neg.lower() in desc_lower:
                neg_total += 15
                breakdown.append(f"Negative: '{neg[:30]}': -15")
        if re.search(r'\b[57]\+?\s*years.*(?:pm|product management|product manager)', desc_lower):
            neg_total += 10
            breakdown.append("5+/7+ years PM exp required: -10")
        neg_total = min(neg_total, 25)
        total -= neg_total
...
def main():
    ...
    # Load dedup keys from previous runs
    queue_dir = os.path.join(PROJECT_DIR, "state/queue")
    ...
    if new_jobs:
        ...
        for i, job in enumerate(new_jobs):
            if job.get("job_description") and len(job["job_description"]) > 200:
                continue  # already have a good description (API sources)
            if job["source"] == "LinkedIn":
                fetch_linkedin_jd(job)
            else:
                ...
                if page_html:
                    ...
                    if meta and len(meta.group(1)) > 50:
                        ...
                        body = re.sub(r'<style[^>]*>.*?</style>', '', body, flags=re.DOTALL)
                        text = strip_html(body)
                        for marker in ["About the job", "Job Description", "Job description", "Description"]:
                            if marker in text:
    ...
    for job in new_jobs:
        ...
        if score >= 65: job["tier"] = "T1"
        elif score >= 45: job["tier"] = "T2"
        elif score >= 30: job["tier"] = "T3"
    ...
    # Save report
    report_dir = os.path.join(PROJECT_DIR, "state/reports")
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(report_dir, f"report_{date_str}.md")
    with open(report_path, 'w') as f:
        f.write(email_body)

    # Save queue (all scored jobs for dedup)
    queue_path = os.path.join(queue_dir, f"jobs_{date_str}.json")
